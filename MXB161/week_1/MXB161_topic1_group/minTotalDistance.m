function [optDist, optx, opty] = minTotalDistance(sx, sy, snames, event)
% [OPTIMAL_DISTANCE, OPTIMAL_X OPTIMAL_Y] = minTotalDistance(SX, SY, NAMES, EVENT)
% finds the optimal destination that minimises the total distance for
...
elseif strcmpi(event, 'coffee')
    f = @(x) sum(sqrt((s(:,1) - x(1)).^2 + (s(:,2) - x(2)).^2));
    d = fminsearch(f, [0 0]);
    n = 1;
else
...
% calculate total distance from all sources to each destination
T = sum(L,1);
...
% title
title_str = sprintf('Total distance travelled = %1.2f', optDist);
if strcmpi(event, 'bbq')
